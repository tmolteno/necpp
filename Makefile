...
# --- WASM target (requires Docker) ---
# Build with:  make wasm
WASM_IMAGE = emscripten/emsdk:4.0.7
WASM_DOCKER = docker run --rm --user $(shell id -u):$(shell id -g) \
                -v $(shell pwd):/src -w /src $(WASM_IMAGE) emcc
...
wasm: $(BUILD_DIR)/config.h
	$(WASM_DOCKER) -std=c++17 -O2 $(INCLUDES) \
		-s WASM=1 \
		...
		--no-entry \
		-o nec2pp.js \
		$(SRC_DIR)/nec_wasm.cpp $(SRC_DIR)/XGetopt.cpp \
		$(LIB_SRCS:%=$(SRC_DIR)/%.cpp)
