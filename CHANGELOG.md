...
### Performance
* **`saoa` shares one `xl*xl` across all five uses:** the Sommerfeld integrand squared `xl` independently for each of `cgam1`, `cgam2` (the two complex `sqrt` arguments), the three `dgam` large-|xl| branches, and the `rho==0` transverse answer. One complex multiply per integrand evaluation is now shared (issue #131). Algebraic identity — byte-identical output on all five `GN 2` decks; benchmarked at ≤0.1% on a noisy laptop, consistent with the multiplication being a small fraction of the transcendental-dominant kernel.
...
* **Sommerfeld ground (`GN 2`) fixed: three defects in the ground-integration path (#130, @KJ7LNW).** Over a `GN 2` Sommerfeld/Norton ground, feed impedances could be wrong by tens to hundreds of ohms; on low antennas the feed resistance even went negative, a passive structure appearing to generate power (`37-3e` reported R = -60.3 Ω where Fortran gives +130.194 Ω). The fixes:
  - **`rom2` subdivision cap bound to structure size:** Both `rom2` copies derived their minimum-interval epsilon and step-halving bound from the segment-plus-patch count (inherited from the original `nec2c` port) instead of NEC-2's routine-local `NM = 65536` constant. Small models over ground received the coarsest integration — exactly where per-segment ground interaction is strongest — and could exhaust the bound before the relative-error test converged, accumulating an unconverged Richardson extrapolation. Both copies now terminate on the `RX = 1e-4` error test instead of an exhausted counter.
  - **`c_ggrid::interpolate` guard inverted:** The cache-opening guard had its two branches exchanged relative to `SUBROUTINE INTRP` — a point at/above the grid-region origin recomputed unconditionally, while a point *below* the origin reused stale coefficients whenever the leftover indices satisfied the proximity test. Interpolation therefore returned different values for the same `(x, y)` depending on request order. Restored the reference `jump`-flag form with indices computed only in the complementary branch.
  - **`efld` thin-wire displacement used `norm` instead of `norm2`:** The Sommerfeld displacement computed `norm()` (a square root) where the reference's `RH=RHOX*RHOX+RHOY*RHOY+RHOZ*RHOZ` followed by `RH=AI/SQRT(RH)` was required, taking the root twice and placing the observation point at the wrong radius on every thin-wire ground evaluation. Now uses `norm2()`.
  Cross-validated against `nec2c`, `nec2dx`, `nec2dxs`, and `xnec2c` over 16 `GN 2` decks: failing comparison points on `nec2++` edges fall from 489 to 208. An order-dependence sweep over 20301 interpolation points found 3657 of 20301 records differing on the unfixed code in reverse order, 0 after.
...
### Tests
* GX symmetry unit tests now validate exact impedance values for both multi-feed indexed access and single-feed
